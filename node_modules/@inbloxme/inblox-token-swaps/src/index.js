...
} = require('./constants/ABI/kyber-proxy-contract');
...
class TokenSwap {
  constructor(wsProvider) {
    ...
    this.kyberNetworkContract = new web3.eth.Contract(
      this.kyberProxyContractABI,
  ...
  async swapTokens({
    ...
    if (srcTokenAddress === ETH_TOKEN_ADDRESS) {
      ...
      const userEthBalance = await this.getTokenBalance(
        ETH_TOKEN_ADDRESS,
        userAdd
      );
    ...
    if (srcTokenAddress !== ETH_TOKEN_ADDRESS) {
      const results = await this.getRates(
        srcTokenAddress,
        dstTokenAddress,
        srcQtyWei
      );
      const srcTokenContract = new web3.eth.Contract(
        erc20Contract,
        srcTokenAddress
      );
  ...
  async getSlippage(srcTokenAddress, dstTokenAddress, srcQty) {
    ...
    const result1 = await this.getRates(
      srcTokenAddress,
      dstTokenAddress,
      quantity1
    );
    const result2 = await this.getRates(
      srcTokenAddress,
      dstTokenAddress,
      quantity2
    );
    const slippage =
      ((result1.expectedRate - result2.expectedRate) * 100) /
      result1.expectedRate;
...
async function signAndSendTransaction({ wallet, pvtKey, rawTx }) {
  switch (wallet) {
    case 'handlename': {
      return rawTx;
    }
...
async function signViaPrivateKey(pvtKey, rawTx) {
  ...
  const txReceipt = await web3.eth
    .sendSignedTransaction(stringTx)
    .catch((error) => error);
